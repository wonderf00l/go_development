...
func main() {
	...
	pool := NewWorkerPool(workers) // запланирвали горутины, которые будут таскать из внутреннего канала джобы

	res := make([]int64, tasks)

	for i := 0; i < tasks; i++ {
		pool.Do(&LongJob{input: int64(i), result: &res[i]}) // пишем джобы в канал
	}
