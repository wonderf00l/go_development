...
func main() {
	go startServer() // запускаем сервер в отдельной горутине, чтобы не заблокироваться в ожидании подлеючений
	// сервер прекращает работу после заверешния работы основного потока выполнения(основной горутины)
