...
import (
	...
	_ "github.com/go-sql-driver/mysql" // под капотом вызывается init(), инициализируется драйвер - обвязка для mysql
...
func getMysql() *sql.DB {
	...
	db, err := sql.Open("mysql", dsn) // только парсинг, ленивое подключение
...
// https://gorm.io/ - полноценный фреймворк для ORM работы с СУБД
