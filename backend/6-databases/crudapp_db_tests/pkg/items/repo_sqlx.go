...
func (repo *RepoSqlx) GetAll() ([]*Item, error) { // более высокоуровневый интерфейс, используется рефлексия под капотом
	...
	return items, nil
} // коннект кладется обратно в пул автоматически
...
func (repo *RepoSqlx) Add(elem *Item) (int64, error) {
	result, err := repo.DB.NamedExec( // можем передать именованную мапу
