...
func (repo *RepoGorm) GetAll() ([]*Item, error) {
	...
	err := repo.DB.Find(&items).Error // передаем интерфейс, реализующий таблицу в контексте gorm
	if err != nil {
		return nil, err
	}
	return items, nil
...
func (repo *RepoGorm) Add(elem *Item) (int64, error) {
	err := repo.DB.Create(elem).Error // gorm соберет запрос из имеющейся структуры, создаст поле в таблице
